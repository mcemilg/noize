...
def apply_cmd(cmd, args):
    ...
    if args.command == CMD_PER:
        ...
        noisy_im = salt_and_pepper(np.array(img), args.probability)
    elif args.command == CMD_GSS:
        noisy_im = gaussian(np.array(img), args.mean, args.var)
    elif args.command == CMD_RAY:
        noisy_im = rayleigh(np.array(img), args.loc, args.scale)
    elif args.command == CMD_ER:
        noisy_im = erlang(np.array(img), args.a, args.loc, args.scale)
    elif args.command == CMD_EXP:
        noisy_im = exponential(np.array(img), args.loc, args.scale)
